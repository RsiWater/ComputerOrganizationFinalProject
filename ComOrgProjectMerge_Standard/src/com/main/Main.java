...
public class Main {
	public static void main (String[] args)
    {
        ...
        for(int cycle = 1;(cycle <= 5)|| !(wb.isNull() && mem.isNull() && ex.isNull() && id.isNull() && If.EOF(If.getPC())) ;cycle++)
        {
            ...
            if(exmem.getBranch()==true && exmem.getBranZero()==true) { ifid.resetAll(); }
            id.startID();
...
//            	System.out.println(id.getexhazardflag());
            }

            if(!id.getexhazardflag() && !id.getmemhazardflag())
            {
                if(exmem.getBranch() && exmem.getBranZero())
                {
                    If.startIF(exmem.getBA());
                    If.printCurrentStage(If.getPC());
                }
                else {
                    If.startIF(If.getPC());
