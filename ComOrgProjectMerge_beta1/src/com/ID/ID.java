	...
	public void startID(int instruction) { //instruction 改成IF/ID物件 或 使用本身的IF/ID
		...
		if(opcode==0) {  //add sub
			if((instruction&63) == 32) {
				idex.setRegIDEX(true, 0, false, false, false, false, true, false); //add 第二位設置aluOp也就是設置要做add或sub運算
			...
			idex.setRegIDEX(false, 2, true, false, true, false, true, true);
			idex.setCurrentInstruction("lw");
			//System.out.println("lw");
		}
		else if(opcode==-1409286144) { //sw
			idex.setRegIDEX(false, 2, true, false, false, true, false, false); //首末為x
			idex.setCurrentInstruction("sw");
			//System.out.println("sw");
		}
		else if(opcode==268435456) { //branch
			idex.setRegIDEX(false, 2, false, true, false, false, false, false); //首末為x
		...
		idex.setPassRegIDEX( 0, reg1, reg2, addOrCon, rt, rd);
	}
